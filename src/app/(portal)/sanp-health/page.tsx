...
export default function SanpHealthPage() {
  ...
  const selectedDetail = selectedRunId ? selectedQuery.data ?? null : latestDetail;
  const selectedReport = selectedDetail?.report ?? null;
  const isLoading = reportsQuery.isLoading
    || latestQuery.isLoading
    || (selectedRunId !== null && selectedQuery.isLoading);
  const fatalError = reportsQuery.error ?? latestQuery.error ?? selectedQuery.error;
        ...
        <div role="status" className="flex items-start gap-3 rounded-lg border border-warning/30 bg-warning-soft px-4 py-3 text-sm text-warning">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs">
              {latestQuery.data.stale_reason ?? syncStatus?.last_error ?? "L'ultima sincronizzazione non è riuscita."}
              {syncStatus?.last_success_at && ` · Ultimo aggiornamento riuscito: ${formatDateTime(syncStatus.last_success_at)}`}
            </p>
          </div>
        </div>
      )}
